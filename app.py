...
def api_my_packages():
    ...
    try:
        cursor.execute("SELECT DISTINCT course_name FROM user_courses WHERE user_id = %s AND status = 'ACTIVE'", (session['user_id'],))
        owned_courses = [r['course_name'] for r in cursor.fetchall()]
        cursor.execute("SELECT DISTINCT course_name FROM user_courses WHERE user_id = %s AND DATE(created_at) = CURDATE()", (session['user_id'],))
...
def get_income_history():
    ...
    user_id = session['user_id']
    bonus_type = request.args.get('type')

    # 2. John Doe (ID: 1) Override
    if str(user_id) == '1':
        dummy_data = {
            'DAILY_ROI': [{'day_count': 1, 'date': '2026-06-01', 'amount': 150.00}],
            'DIRECT_SPONSOR': [{'day_count': 1, 'date': '2026-06-01', 'amount': 500.00}],
            'BINARY_MATCH': [{'day_count': 1, 'date': '2026-06-04', 'amount': 750.00}],
            'REPURCHASE': [{'day_count': 1, 'date': '2026-06-06', 'amount': 250.00}],
            'ROYALTY': [{'day_count': 1, 'date': '2026-06-07', 'amount': 1000.00}],
            'CASHBACK': [{'day_count': 1, 'date': '2026-06-08', 'amount': 150.00}] 
        }
        return jsonify({'status': 'success', 'data': dummy_data.get(bonus_type, [])})

    # 3. Actual MySQL Logic for everyone else
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        
        # Mapping frontend types to database transaction_type values
        type_map = {
            'DAILY_ROI': 'staking_bonus', 
            'DIRECT_SPONSOR': 'direct_sponsor',
            'BINARY_MATCH': 'binary_match',
            'REPURCHASE': 'self_repurchase',
            'ROYALTY': 'captain_royalty',
            'CASHBACK': 'cashback'
        }
        db_type = type_map.get(bonus_type, bonus_type.lower())

        # SQL Query for your wallet_transactions table
        query = """
        ...
        """
        cursor.execute(query, (user_id, db_type))
        ...
        return jsonify({'status': 'error', 'error': 'Database query failed'}), 500
...
def api_withdraw_me():
    # Use session.get to safely check the name
    if session.get('user_name') == "John Doe":
        return jsonify({
            "status": "success", 
            "balance": 56700.00, 
            "history": [
                {"date": "01 Jun 2026", "amount": 2500.00, "status": "APPROVED"},
                {"date": "04 Jun 2026", "amount": 1200.00, "status": "COMPLETED"}
            ]
        })
    else:
        # NO BRACES HERE! Just indent the code under the 'else:'
        if 'user_id' not in session: 
            return jsonify({"status": "error", "message": "Unauthorized"}), 401
        ...
        # 2. Get History
        cursor.execute("""
            SELECT created_at as date, request_amount as amount, status 
            FROM withdrawals 
            WHERE user_id = %s 
            ORDER BY created_at DESC
        """, (user_id,))
        ...
        return jsonify({"status": "success", "balance": float(balance), "history": history})
...
def get_team_list(category):
    ...
    # CORRECTED QUERY: Joining on u.id = uc.user_id
    query = """
        SELECT u.user_code as id, u.full_name as name, uc.course_name as course 
        FROM users u 
        LEFT JOIN user_courses uc ON u.id = uc.user_id 
        WHERE u.sponsor_id = %s
    """
    
    # Add filtering based on the clicked category
    if category == 'active':
        query += " AND u.is_active = 1"
    elif category == 'inactive':
        query += " AND u.is_active = 0"
    elif category == 'left':
        query += " AND u.leg = 'left'"
    elif category == 'right':
        query += " AND u.leg = 'right'"
    # 'directs' uses the basic sponsor_id query already defined
